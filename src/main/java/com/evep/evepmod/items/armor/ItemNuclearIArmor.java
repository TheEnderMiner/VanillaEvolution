...
public class ItemNuclearIArmor extends ItemArmor implements IHasModel{

	public ItemNuclearIArmor(String name, ArmorMaterial materialIn, int renderIndexIn, EntityEquipmentSlot equipmentSlotIn) {
	...
	public boolean getIsRepairable(ItemStack armor, ItemStack stack){
		return stack.getItem() == VanillaEvolutionItems.uranium_ingot;
	}
	...
	{
		{
			if(player.inventory.armorInventory.get(0).getItem() == VanillaEvolutionItems.nucleari_boots)
			{
				player.addPotionEffect(new PotionEffect(MobEffects.SPEED, 12*20, 2));
			}
			{
				if(player.inventory.armorInventory.get(1).getItem() == VanillaEvolutionItems.nucleari_leggings)
				{
					player.addPotionEffect(new PotionEffect(MobEffects.HASTE, 12*20, 2));
				}
			}
			{
				if(player.inventory.armorInventory.get(2).getItem() == VanillaEvolutionItems.nucleari_chestplate)
				{
					player.addPotionEffect(new PotionEffect(MobEffects.FIRE_RESISTANCE, 12*20, 0));
				}
			}
			{
				if(player.inventory.armorInventory.get(3).getItem() == VanillaEvolutionItems.nucleari_helmet)
				{
		...
		if(player.inventory.armorInventory.get(0) != null &&
			...
			{
				if(player.inventory.armorInventory.get(0).getItem() == VanillaEvolutionItems.nucleari_boots &&
					player.inventory.armorInventory.get(1).getItem() == VanillaEvolutionItems.nucleari_leggings && 
					player.inventory.armorInventory.get(2).getItem() == VanillaEvolutionItems.nucleari_chestplate &&
					player.inventory.armorInventory.get(3).getItem() == VanillaEvolutionItems.nucleari_helmet)
				{
